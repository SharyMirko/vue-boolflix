<template>
  <div id="app">
    <HeaderBoolflix
      @caunt="caunt = $event"
      @searchStr="searchRequest($event)"
    />
    <MainBoolfix
      :arrMovie="resultsMovie"
      :caunt="caunt"
      :arrSeries="resultsSeries"
    />
  </div>
</template>
...
export default {
  ...
  methods: {
    searchRequest(str) {
      axios
        .get(
          "https://api.themoviedb.org/3/search/movie?api_key=1814a5181699a3f32f15c63dc0665bd9&language=it-IT&query=" +
            str
        )
        .then((res) => {
      ...
      axios
        .get(
          "https://api.themoviedb.org/3/search/tv?api_key=1814a5181699a3f32f15c63dc0665bd9&language=it-IT&query=" +
            str
        )
        .then((resp) => {
